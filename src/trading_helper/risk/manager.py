...
def calculate_position_size(portfolio_value: float, risk_percent: float, entry_price: float, stop_price: float) -> PositionSizing:
    ...
    return PositionSizing(quantity=quantity, capital_required=round(quantity * entry_price, 2), maximum_loss=round(quantity * risk_per_share, 2), risk_per_share=round(risk_per_share, 4))
